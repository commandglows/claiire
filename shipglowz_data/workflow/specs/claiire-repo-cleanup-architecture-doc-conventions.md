...
owner: "Diane"
confidence: high
user_story: "En tant qu'operatrice du monorepo Claiire, je veux une structure site/app et une documentation Shipglowz canonique, afin que les agents puissent maintenir le projet sans confusion, doublon ni dette de rangement."
risk_level: high
security_impact: yes
docs_impact: yes
linked_systems:
  ...
  - "site/public/shipglowz-inspector.js"
...
depends_on:
  ...
  - artifact: "shipglowz_data/business/business.md"
    artifact_version: unknown
    required_status: unknown
  - artifact: "shipglowz_data/editorial/site/guidelines.md"
    artifact_version: unknown
    required_status: unknown
  - artifact: "shipglowz_data/technical/site/CLAUDE.md"
    artifact_version: unknown
    required_status: unknown
supersedes: []
evidence:
  ...
  - "User correction 2026-06-29: Markdown/documentation must be regrouped into shipglowz_data following conventions; branding remains centralized."
  - "User correction 2026-06-29: app/_bmad must be moved to the current user's home on the server and removed from this repo."
  - "User correction 2026-06-29: app/Inspiration must be classified into Shipglowz data, with final handling reviewed."
  ...
  - "Audit 2026-06-29: site layouts load Shipglowz debug scripts globally and public inspector files contain an ImgBB API key."
...
- Root legacy trackers/docs are removed or converted to allowed compatibility symlinks only where Shipglowz conventions allow them.
- `shipglowz_data/` contains the canonical docs with metadata frontmatter and a clear shared/site/app split.
- The site no longer exposes Shipglowz debug/inspector scripts or hardcoded public upload keys in production assets.
...
4. Classify `app/Inspiration/` before moving it into the appropriate `shipglowz_data` reference/research structure or a review quarantine.
5. Canonicalize all Markdown/documentation into `shipglowz_data/`, keeping branding centralized and splitting app/site-specific governance by surface.
...
- Classify `app/Inspiration/` material before moving it to a governance/reference location under `shipglowz_data/` or a review quarantine.
- Canonicalize docs:
  - Shared branding stays centralized under `shipglowz_data/branding/`.
  - Site docs live under `shipglowz_data/{business,editorial,technical,workflow}/site/`.
  - App docs live under `shipglowz_data/{business,product,technical,workflow}/app/`.
...
- Add required Shipglowz frontmatter metadata to canonical docs when missing.
- Remove or gate public Shipglowz inspector/Eruda scripts and hardcoded upload keys from site assets.
...
- Do not create parallel documentation doctrines outside `shipglowz_data/`.
...
- Monorepo governance root is the repository root `shipglowz_data/`.
...
surface: mixed monorepo (`Astro/Vercel/Clerk` site + `Expo/React Native/Convex/Clerk` app + Shipglowz governance docs)
...
- SC-06 canonical docs exist under `shipglowz_data/` with allowed shared/site/app split
...
- `rg -n "IMGBB_API_KEY|shipglowz-inspector|shipglowz-eruda|buildflowz-inspector|eruda|api.imgbb.com" site/src site/public` proves public debug/upload scripts are removed or gated
...
- `shipglowz_data/branding/branding.md` is the shared branding source of truth.
- Surface-specific docs must not duplicate shared branding; they may reference it.
- `shipglowz_data/workflow/specs/` is the chantier registry.
- Root `TASKS.md` and `AUDIT_LOG.md` are legacy and must not be canonical.
- Public site bundles must not include debug upload tooling, hardcoded API keys, or Shipglowz internal inspectors.
...
- `shipglowz_data/branding/branding.md`: shared brand contract with metadata frontmatter.
- `shipglowz_data/business/business.md`: shared business contract with metadata.
- `shipglowz_data/editorial/site/guidelines.md`: site editorial/copy contract with metadata.
- `shipglowz_data/technical/site/README.md`: site development guide updated to pnpm, Node, Astro 6, current domain, and real commands.
- `shipglowz_data/technical/site/CLAUDE.md`: site technical agent guide updated to actual package/build/auth/runtime behavior.
- app-specific business details merged into `shipglowz_data/business/business.md`, sourced from `app/BUSINESS.md`.
- `shipglowz_data/branding/branding.md`: shared app/site brand reference; app-specific visual guidance may live under `shipglowz_data/product/app/` or `shipglowz_data/technical/app/` only if it does not fork shared brand truth.
- `shipglowz_data/technical/app/CLAUDE.md`: app technical guide migrated from `app/CLAUDE.md`, corrected so it no longer points to site docs as app source of truth.
- `shipglowz_data/workflow/app/` and `shipglowz_data/workflow/site/`: surface-specific trackers only where needed.
- `shipglowz_data/research/app/` for classified inspiration indexes and reusable app references, plus a clearly named quarantine/review subpath if some items cannot be decided immediately.

# Edge Cases

- `app/app/TASKS.md` is a symlink to `/home/claude/shipglowz_data/projects/app/TASKS.md`; do not blindly move/delete without deciding whether it is legacy and whether external Shipglowz data still uses it.
...
- [ ] Task 1: Freeze current state and create a migration inventory.
  - File: `shipglowz_data/workflow/specs/claiire-repo-cleanup-architecture-doc-conventions.md`
...
- [ ] Task 5: Classify `app/Inspiration/` before relocation.
  ...
  - Depends on: Task 1.
  - Validate with: classification report under `shipglowz_data/research/` or `shipglowz_data/workflow/reviews/`; `git ls-files app/Inspiration | wc -l` after approved relocation.
...
- [ ] Task 6: Migrate Markdown and governance docs into `shipglowz_data`.
  - File: `app/BUSINESS.md`, `app/BRANDING.md`, `app/GUIDELINES.md`, `app/CLAUDE.md`, `app/DEV_IDEAS.md`, `app/README.md`, `app/app/AUDIT_LOG.md`, `app/app/CHANGELOG.md`, root `TASKS.md`, root `AUDIT_LOG.md`, and existing `shipglowz_data/**`.
  - Action: Move or merge docs into canonical shared/site/app paths with Shipglowz frontmatter; remove legacy originals except allowed compatibility files/symlinks.
...
- [ ] Task 7: Update docs references and stale commands.
  - File: `shipglowz_data/technical/site/README.md`, `shipglowz_data/technical/site/CLAUDE.md`, `shipglowz_data/technical/app/CLAUDE.md`, package READMEs if retained.
  ...
  - Depends on: Task 6.
  - Validate with: `rg -n "npm install|npm run|app/app|app/_bmad|app/Inspiration|claiire.com|claiire.fr|site/CLAUDE|BUSINESS.md|GUIDELINES.md" shipglowz_data app site -g '*.md'`.
...
- [ ] Task 8: Remove public debug/security leaks from the site.
  - File: `site/src/layouts/DocsLayout.astro`, `site/src/layouts/StaticPageLayout.astro`, `site/public/shipglowz-inspector.js`, `site/public/buildflowz-inspector.js`, `site/public/shipglowz-eruda.js`.
  ...
  - Depends on: Task 1.
  - Validate with: `rg -n "IMGBB_API_KEY|api.imgbb.com|shipglowz-inspector|buildflowz-inspector|shipglowz-eruda|eruda" site/src site/public`.
...
- [ ] Task 10: Final repository hygiene check.
  ...
  - Validate with: `git status --short`; `git ls-files app/_bmad app/Inspiration app/app`; `git ls-files | rg '(^|/)node_modules/|(^|/)dist/|(^|/)\\.astro/|(^|/)\\.vercel/'`; `find shipglowz_data -maxdepth 3 -type f | sort`.
...
- [ ] CA 5: Given the repo after migration, when a fresh agent searches for business, technical, editorial, workflow, or app guidance, then canonical files are under `shipglowz_data/` and legacy root/app docs are gone or allowed compatibility links only.
- [ ] CA 6: Given shared brand guidance, when app/site docs reference branding, then they point to `shipglowz_data/branding/branding.md` and do not duplicate/fork the shared brand contract.
- [ ] CA 7: Given the public site source, when `rg` searches for Shipglowz inspector, Eruda, ImgBB, or hardcoded upload keys, then no production-loaded public asset exposes those debug/upload tools.
...
- [ ] CA 12: Given docs metadata proof, when canonical Markdown governance files are sampled, then they include Shipglowz frontmatter with artifact, schema version, status, scope, owner/confidence/risk where applicable, and next step/review fields.
...
- Use doc-location scans to prove Shipglowz governance conventions.
...
- Read first:
  ...
  - `app/package.json`
  - `app/app/package.json`
  - `shipglowz_data/technical/site/CLAUDE.md`
  - `app/CLAUDE.md`
  - `site/src/layouts/DocsLayout.astro`
  - `site/src/layouts/StaticPageLayout.astro`
...
- business and product stay on one shared documentation surface under `shipglowz_data/business/`; site/app separation belongs inside files, not in duplicated folder trees.
...
- Documentation/security proof passes: canonical docs were regrouped under `shipglowz_data/`, the shared business surface was consolidated, and public Shipglowz inspector/upload script references were removed from `site/src` and `site/public`.
...
| 2026-06-30 14:59:32 UTC | 300-sf-docs + 307-sf-skills-refresh | GPT-5 Codex | Normalized the Claiire branding bundle under `shipglowz_data/branding/` and refreshed Shipglowz governance references, bootstrap rules, and metadata linting to recognize the richer branding architecture | Implemented. Claiire now has a governed brand bundle with dedicated files for voice, messaging, visual identity, rules, and assets; Shipglowz active references now resolve the canonical branding path and bundle shape | /106-sf-fix claiire-post-cleanup-proof-gaps |
