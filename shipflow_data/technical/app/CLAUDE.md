# Claiire App - Technical Reference

## Scope

This file documents the mobile app surface only.

## Package Root

- package root: `app/`
- Expo Router routes: `app/app/`

## Source Of Truth

- shared business/product: `shipflow_data/business/business.md`
- shared branding: `shipflow_data/branding/branding.md`
- app workflow history: `shipflow_data/workflow/app/`
- inspiration archive: `shipflow_data/research/app-inspiration/`

## Product Summary

Claiire app is a mobile wellness companion built around:

- on-device AI companionship
- gamified progression
- habit and crisis-support flows
- privacy-forward positioning

## Stack

- Expo 55
- TypeScript
- Expo Router
- Clerk Expo
- Convex
- Zustand
- Jest / jest-expo

## Constraints

- Expo Go is not enough for the full native surface
- Convex is the persistent data source of truth
- Zustand is UI state only
- safe vocabulary rules from `shipflow_data/technical/app/guidelines.md` apply everywhere

## Validation

- workspace package path: `app/`
- package manifest: `app/package.json`
- tests: `pnpm --dir app test` or equivalent local package command
